...
\begin{itemize}
    \item Build confidence in the correctness of the **repetition counting logic**.
    \item Verify the **accuracy** of the biomechanical angle calculations.
    \item Ensure the system operates in **real-time**.
\end{itemize}
...
\begin{itemize}
    ...
    \item Review by the supervisor (Dr. Down).
...
Implementation verification will focus on **Manual Unit Testing** of the core logic and **Automated Testing** where feasible.
\begin{itemize}
    \item **Static Analysis**: Code reviews and linting.
    \item **Dynamic Testing**: Manual execution of logic test cases (see Section 4).
\end{itemize}

\subsection{Automated Testing and Verification Tools}
The following tools will be used:
\begin{itemize}
    \item **Pytest**: For future automated unit tests.
    \item **GitHub Actions**: For Continuous Integration (CI).
    \item **MediaPipe**: External library (assumed verified) for pose estimation.
\end{itemize}

\subsection{Software Validation}
Validation will compare the system's output against ground truth:
\begin{itemize}
    \item **Geometric Verification**: Comparing calculated angles against physical goniometer measurements (Ground Truth).
\end{itemize}
...
Unit testing will focus on the **Analysis Module** (State Machine), as it contains the critical business logic. The UI and Camera modules are tested via System Testing.
