...
2026-04-20 & 1.2 & Addressed Yibing Mei peer-review feedback: disambiguated ``MAE'' vs.\ mean frame-to-frame angle delta in the dynamic benchmark (\S\ref{sec:nfunc}); consolidated duplicated per-test subsections in \S\ref{sec:unit} to reduce overlap with \S\ref{sec:func}\\
\bottomrule
\end{tabularx}
...
front-45, side, back, back-45). The primary metric is the mean absolute
frame-to-frame angle delta, denoted
$\overline{|\Delta\theta|} = \frac{1}{N{-}1}\sum_{i=1}^{N-1}|\theta_i - \theta_{i-1}|$
(referred to as \texttt{mean\_abs\_delta} in the code). This is a
\emph{stability} metric, not an error-versus-ground-truth metric: lower values
indicate a smoother, less noisy angle signal across consecutive frames.

\noindent\textbf{Note on terminology.} ``MAE'' in this report is used only in
the T8 analytical results (Table~\ref{tab:t8}), where it denotes
Mean Absolute Error against a closed-form ground-truth angle. The dynamic
benchmark below does \emph{not} have a per-frame ground truth available, so
$\overline{|\Delta\theta|}$ (frame-to-frame stability) is reported instead.

\begin{table}[H]
\centering
\begin{tabular}{l c c c}
\toprule
\textbf{Backend} & \textbf{Avg.\ Latency (ms)} & \textbf{Knee $\overline{|\Delta\theta|}$ ($^\circ$)} & \textbf{Elbow $\overline{|\Delta\theta|}$ ($^\circ$)}\\
\midrule
...
\bottomrule
\end{tabular}
\caption{Offline Benchmark --- Mean Absolute Frame-to-Frame Angle Delta ($\overline{|\Delta\theta|}$) by Backend. Lower is more stable; this is \emph{not} error vs.\ ground truth.}
\label{tab:offline}
\end{table}

\texttt{mediapipe\_2d} achieves the best throughput ($\sim 25$~FPS) and
knee-angle stability (lowest $\overline{|\Delta\theta|}$). This data justifies
the design decision in M4 to use MediaPipe 2D as the production backend.
...
\paragraph{Note on section layout.}
Tests T1, T2, and T5 are reported in Section~\ref{sec:func} (R4 evaluation,
Table~\ref{tab:validity}) to avoid duplication. The subsections below cover
only tests whose results require additional detail (state-transition traces,
geometric accuracy, signal smoothing, multi-rep accumulation) beyond what
Table~\ref{tab:allresults} already summarises.
