...
\multirow{4}{0.3\textwidth}{Behaviour-Hiding} & \mref{m2}\\
& \mref{m3}\\
& \mref{m6}\\
& \mref{m7}\\
\midrule

\multirow{3}{0.3\textwidth}{Software Decision} & \mref{m4}\\
& \mref{m5}\\
& \mref{m8}\\
\bottomrule
...
Each module's design decision is driven by a corresponding SRS requirement.
For example, the decision to use a Kalman filter (M8) was made to satisfy
the stability requirement for real-time angle tracking. The decision to use
MediaPipe (M4) was made to satisfy the accuracy requirements without
requiring specialized hardware (e.g., depth cameras).
...
\begin{description}
\item[Secrets:] The filtering algorithms used to remove noise from landmark coordinates.
\item[Services:] Provides a stable, non-jittery signal for rep counting and display.
\item[Implemented By:] FitCoachAR
\end{description}
...
\midrule
R1 & \mref{m1}, \mref{m3}, \mref{m4}, \mref{m7}\\
R2 & \mref{m4}, \mref{m5}\\
R3 & \mref{m6}, \mref{m7}\\
R4 & \mref{m2}, \mref{m7}\\
R5 & \mref{m8}\\
...
\begin{itemize}
  ...
  \item M6 (Exercise State) \emph{uses} M5 (Kinematic Engine), M8 (Signal Smoothing)
  \item M5 (Kinematic Engine) \emph{uses} M4 (Pose Tracking)
  \item M4 (Pose Tracking) \emph{uses} M3 (Video Formatting)
