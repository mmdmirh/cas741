...
The end user is expected to be a fitness enthusiast or a patient undergoing physical therapy. They are assumed to have basic computer literacy.

\subsection{System Constraints}
\begin{itemize}
    \item The system is deployed as a \textbf{web application} for maximum portability. (Note: The specific architectural decision to make it a web application acts as a constraint on the implementation phase, though it could ideally be postponed to the design document).
    \item The system depends on a \textbf{pose estimation backend} (e.g., MediaPipe or MoveNet) for raw keypoint extraction. 
\end{itemize}
...
The decisions captured in this document are based on the goal of creating a scientifically rigorous and accessible exercise form analysis tool using 2D computer vision. The constraints and assumptions selected reflect the trade-off between widespread accessibility (using generic webcams instead of specialized hardware) and preserving geometric accuracy.
