...
The decisions captured in this document are based on the requirements of the CAS 741 course project and the goal of creating a scientifically rigorous squat analysis tool using 2D computer vision.
...
\hline        
	& \tref{TM:VectorAngle}& \ddref{DD_Landmarks}& \ddref{DD_Landmarks_Arm} & \dref{GD_KneeAngle} & \dref{GD_ElbowAngle} \\
\hline
...
\hline
	& \ddref{DD_Landmarks}& \ddref{DD_Landmarks_Arm} & \dref{GD_KneeAngle}& \dref{GD_ElbowAngle} & \iref{IM_SquatValid} & \iref{IM_BicepCurlValid} \\
\hline
\rref{R_Process}       & X & X & & & & \\ \hline
\rref{R_Calc}          & & & X & X & & \\ \hline
