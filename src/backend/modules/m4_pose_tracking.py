...
Secret            : Choice of ML tracker natively built-in (MediaPipe 2D).
Service           : Converts a raw video frame into an array of 2D/3D
                    (x, y [, z]) landmarks with visibility scores, and orchestrates
                    the pipeline with M5, M6, and M8 for form analysis and rep counting.

MIS Syntax
----------
  process_frame(frame: ndarray) → Optional[Dict[str, Any]]
...
if TYPE_CHECKING:
    from session import FormSnapshot, WorkoutSession
    from coaches import FormChecker
...
from modules.m8_signal_smoothing import KalmanLandmarkSmootherAdapter as KalmanLandmarkSmoother


# ═══════════════════════════════════════════════════════════════════════════════
#  BASE INTERFACES (From former base.py)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ESTIMATOR IMPLEMENTATION (From former mediapipe_estimator.py)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ORCHESTRATOR IMPLEMENTATION (From former pose_processor_2d.py)
# ═══════════════════════════════════════════════════════════════════════════════
...
class PoseProcessor(PoseBackend):
    ...
    def __init__(self, estimator: PoseEstimator):
        ...
        from calibration_v2 import CalibrationStore
    ...
    def _analyze_rep_buffer(self, exercise: str) -> Optional[FormSnapshot]:
        ...
        from session import FormSnapshot
    ...
    def handle_command(self, command_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        from coaches import FormChecker
        from calibration_v2 import CalibrationParams, calibrate_from_landmarks
        from session import WorkoutSession
        ...
        if command == "next_set":
            if not self.active_session:
                return {"event": "session_error", "message": "No active session. Start a session first."}
        ...
        if command == "get_session_progress":
            if not self.active_session:
                return {"event": "session_progress", "active": False, "progress": None}
            return {"event": "session_progress", "active": True, "progress": self.active_session.get_progress()}
        
        if command == "end_session":
            if not self.active_session:
                return {"event": "session_error", "message": "No active session to end."}
            
            self.active_session.finish()
            summary = self.active_session.to_dict()
            self.active_session = None
            return {"event": "session_ended", "session": summary}
    ...
    def process_frame(self, frame_bgr: np.ndarray) -> Optional[Dict[str, Any]]:
        ...
        smoothed_objs = self.landmark_smoother.smooth_landmarks(raw_landmarks)
